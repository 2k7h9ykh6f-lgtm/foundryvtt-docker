...
| `CONTAINER_LOG_LEVEL` | Set the log verbosity for container utility scripts.  Valid values are `debug`, `info`, `warn`, `error`, and `quiet`.  Takes precedence over `CONTAINER_VERBOSE` when both are set. | `info` |
| `CONTAINER_VERBOSE` | Set to `true` to enable verbose (debug) logging for the container utility scripts.  Equivalent to `CONTAINER_LOG_LEVEL=debug`.  Kept for backward compatibility; prefer `CONTAINER_LOG_LEVEL`. | `false` |
